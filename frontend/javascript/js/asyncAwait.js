 ...
 * This is an asynchronous function where await drive the order of the process
 * we can add more than one film with few line 
 * and execute .then() statement à the end to display the list of films
 *
 */
async function requestAllNow() {
  try {
    await createNewFilm({
      title: "Deadpool 2",
      description: "He is the craziest hero !",
    });
    await createNewFilm({
      title: "Superman",
      description: "He is the strongest hero !",
    });
    await createNewFilm({
      title: "Spiderman",
      description: "He is just a sticky spider !",
    }).then(getFilms);
  } catch (error) {
    displayError(error);
  }
